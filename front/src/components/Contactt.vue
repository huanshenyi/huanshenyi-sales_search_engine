...
<script lang="ts">
    ...
    import { getCheckServer, getCheckProject, getCheckCrawlerList} from "@/api/api"
    ...
    export default class Contactt extends Vue{
        ...
        checkServer(){
            //クローラー サーバーの状態を取得
            getCheckServer().then((res:any)=>{
                this.ServerStatus.push(res.data)
            })
        }
        checkProject(){
            // アップロードされたプロジェクトを確認
            getCheckProject().then((res:any)=>{
        ...
        checkCrawlerList(){
            //プロジェクト内にあるクローラー を確認
            getCheckCrawlerList().then((res:any)=>{
                if(res.data.status === "ok"){
